...
async function getTracksAudioFeatures(trackIds: string[]): Promise<any[]> {
  const token = await getSpotifyToken();
  
  // Return mock data if using mock token or if API access fails
  if (token === "mock_token") {
    console.log('Using mock data for audio features due to missing API credentials');
    
    // More realistic BPM values for DJ music
    const bpmRanges = [
      98,  // House
      105, // Tech House
      128, // Techno
    ...
    return trackIds.map((id, index) => {
      // Generate BPM values around common DJ music BPM values
      const baseBpm = bpmRanges[index % bpmRanges.length];
      // Add small natural variation (-2 to +2 BPM)
      const bpmVariation = Math.floor(Math.random() * 5) - 2;
      
      return {
        id,
        tempo: baseBpm + bpmVariation,
        key: (index % 12),
        mode: index % 2,
        energy: 0.65 + (index % 4) * 0.05,
        danceability: 0.7 + (index % 3) * 0.05
      };
    });
  }
  ...
  try {
    ...
    console.error('Error fetching audio features from Spotify API:', error);
    console.log('Falling back to mock data for audio features');
    // Return mock data as fallback when API fails
    // More realistic BPM values for DJ music
    const bpmRanges = [
      98,  // House
